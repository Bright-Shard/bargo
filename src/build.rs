...
pub fn build_crate(
	...
	println!("Bargo: Compiling crate `{}`...", pkg.name);
	...
	args.push(build_ctx.ctx.target_dir.to_str().unwrap());
...
fn build_prebuilds(
	...
	if let Some(prebuilds) = pkg.get("prebuild") {
		...
		for (pkg, overrides) in prebuilds.iter() {
			if build_ctx.build_list.contains(pkg) {
				continue;
			}
